...
def main():
    ...
    numperprocess = pixelheight*pixelwidth
    ...
    if id ==0:
        ...
        for n in range(1, mpi.size):
            comm.send[(next_row_to_assign,skystart[next_row_to_assign:(next_row_to_assign+1)*skypixelwidth*3],telestart[next_row_to_assign:(next_row_to_assign+1)*pixelwidth*3],color[next_row_to_assign:(next_row_to_assign+1)*pixelwidth]),dest=n)
            next_row_to_assign+=1
            workers_running+=1
            
        while workers_running>0:
            result,status=mpi.recv(mpi.ANY_SOURCE,tag=0)
            worker_id = status.source
            row_completed,skystartrow,telestartrow,colorrow = result
            for index in range(pixelwidth):
                if(colorrow[index]==1):
                    telepixels[telestartrow[index]:telestartrow[index]+3]=skypixels[skystartrow[index]:skystartrow[index]+3]
                else:
                    telepixels[telestartrow[index]]=255 #leave other two indices zero,red    
            
            if next_row_to_assign < pixelheight:
                comm.send((next_row_to_assign,skystart[next_row_to_assign:(next_row_to_assign+1)*skypixelwidth*3],telestart[next_row_to_assign:(next_row_to_assign+1)*pixelwidth*3],color[next_row_to_assign:(next_row_to_assign+1)*pixelwidth]),dest=n)
                next_row_to_assign +=1
            else:
                comm.send(-1,worker_id)
                workers_running-=1

    else: #id not zero
        while True:
            data, status = comm.recv(mpi.ANY_SOURCE)
            next_row_to_assign,skystartrow,telestartrow,colorrow = data
            if next_row_to_assign == -1:
                break
            for index in range(pixelwidth):
            #index=xpix+ypix*pixelwidth
            ...
            comm.send((next_row_to_assign,skystartrow,telestartrow,colorrow,dest=0)
...
#    skystartall =comm.gather(skystart, root=0)
#    telestartall =comm.gather(telestart, root=0)
#   colorall = comm.gather(color, root=0)
