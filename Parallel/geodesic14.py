...
def main():
    ...
    if mpi.rank ==0:
        ...
        for n in range(1, mpi.size):
            mpi.send[skystart[next_row_to_assign:(next_row_to_assign+1)*skypixelwidth*3],dest=n,tag=0)
            mpi.send[telestart[next_row_to_assign:(next_row_to_assign+1)*pixelwidth*3],dest=n,tag=1)
            mpi.send[color[next_row_to_assign:(next_row_to_assign+1)*pixelwidth],dest=n,tag=2)
            next_row_to_assign+=1
            workers_running+=1
            
        while workers_running>0:
            skyrow,skystatus=mpi.recv(mpi.ANY_SOURCE,tag=0)
            telerow,telestatus=mpi.recv(mpi.ANY_SOURCE,tag=1)
            colorrow,colorstatus=mpi.recv(mpi.ANY_SOURCE,tag=2)
            sky_id = skystatus.source
            tele_id = telestatus.source
            color_id = colorstatus.source
